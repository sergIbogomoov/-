...
&НаСервере
Процедура ОтправитьНаСервере(АдресХранилища)
	Данные = РеквизитФормыВЗначение("Объект");
	Данные.ОтправитьНаСервере(АдресХранилища);
КонецПроцедуры

&НаКлиенте
Процедура Отправить(Команда)
	АдресХранилища = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ЭтотОбъект.Файл), ЭтаФорма.УникальныйИдентификатор);
	ОтправитьНаСервере(АдресХранилища);
КонецПроцедуры
...
&НаСервере
Процедура ВыбратьИзРегистраНаСервере()
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.Расш1_XMLПередача КАК Расш1_XMLПередача
		|ГДЕ
		|	Расш1_XMLПередача.ДатаЗагрузки МЕЖДУ &ДатаНачала И &ДатаОкончания";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ВрФайл = ПолучитьИмяВременногоФайла("xml");
		  		  //ФайлXML.УстановитьТипФайла(КодировкаТекста.UTF8);
		  ФайлXML.УстановитьТекст(ВыборкаДетальныеЗаписи.xml);
		  
		  Попытка
		  	ФайлXML.Записать(ВрФайл,КодировкаТекста.UTF8,Символы.ВК+Символы.ПС);
		Исключение
			Ош = ИнформацияОбОшибке();
			продолжить;
			КонецПопытки;
		
		 АдресХранилища = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ВрФайл));
		 Данные.Инициализация(АдресХранилища, Ложь);
		 УдалитьФайлы(ВрФайл);
	КонецЦикла;
...
&НаКлиенте
Процедура ВыбратьИзРегистра(Команда)
	ВыбратьИзРегистраНаСервере();
КонецПроцедуры
