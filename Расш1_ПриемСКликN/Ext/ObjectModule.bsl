...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия", "1.0.0.9");
...
Процедура Инициализация(АдресХранилища=Неопределено, БратьИзОчереди = Истина) Экспорт
	...
	ИначеЕсли БратьИзОчереди Тогда
	...
	Пока Истина Цикл
		...
		Если ВрФайл = Неопределено И БратьИзОчереди Тогда
		...
		Для Каждого message Из ДокументDOM.ДочерниеУзлы Цикл
			...
			ИначеЕсли message.Атрибуты.Количество() > 0 И message.Атрибуты[0].Значение = "Справочник.ЗапросИнформацииПоКонтрагенту" Тогда
				...
				Для Каждого элементы Из message.ДочерниеУзлы Цикл
			...
			ИначеЕсли message.Атрибуты.Количество() > 0 И message.Атрибуты[0].Значение = "Документ.СчетПокупателю" Тогда
				Для Каждого элемент Из message.ДочерниеУзлы Цикл
					...
					КонецЕсли;
				КонецЦикла;
			ИначеЕсли message.Атрибуты.Количество() > 0 И message.Атрибуты[0].Значение = "ЗапросНаУдалениеЗаявкиНаРасходованиеДС" Тогда
				Для Каждого элемент Из message.ДочерниеУзлы Цикл
					XML_Документ_УдаленимеДокументаЗаявкиНаРасходованиеДС(элемент, Ошибки, УИДФайла);
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;	
...
Процедура XML_Документ_УдаленимеДокументаЗаявкиНаРасходованиеДС(Элементы, Ошибки, УИДФайла)
	Для Каждого КлючевыеСвойства Из Элементы.ДочерниеУзлы Цикл
		КолЗаписей = 0;
		Для Каждого элемент Из КлючевыеСвойства.ДочерниеУзлы Цикл
			Если элемент.ИмяУзла = "КодВПрограмме" Тогда
				Для Каждого ПодЭлемент Из элемент.ДочерниеУзлы Цикл
					КодВПрограмме = ПодЭлемент.ТекстовоеСодержимое;	
					КолЗаписей = КолЗаписей + 1;
				КонецЦикла;
			ИначеЕсли элемент.ИмяУзла = "СтатусУдаления" Тогда
				Для Каждого ПодЭлемент Из элемент.ДочерниеУзлы Цикл
					СтатусУдаления = XMLЗначение(тип("Число"),ПодЭлемент.ТекстовоеСодержимое);	
					КолЗаписей = КолЗаписей + 1;
				КонецЦикла;
			ИначеЕсли элемент.ИмяУзла = "Пользователь" Тогда
				Для Каждого ПодЭлемент Из элемент.ДочерниеУзлы Цикл
					Пользователь = XMLЗначение(тип("Число"),ПодЭлемент.ТекстовоеСодержимое);	
					КолЗаписей = КолЗаписей + 1;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;	
		
		Если КолЗаписей <> 3 Тогда
			Ошибки.Добавить("ЗапросНаУдалениеЗаявкиНаРасходованиеДС, не все ключевые поля присутствуют. (" + УИДФайла + ")");
			Возврат;
		КонецЕсли;
		
		Если СтатусУдаления = 2 Тогда
			РезультатОбработки = "";
			Попытка
				УИД = Новый УникальныйИдентификатор(КодВПрограмме);
				ДокументаЗаявкиНаРасходованиеДС = Документы.ЗаявкаНаРасходованиеДенежныхСредств.ПолучитьСсылку(УИД);		
				
				ДокОбъект = ДокументаЗаявкиНаРасходованиеДС.ПолучитьОбъект();
				Если ДокОбъект = Неопределено Тогда
					ДокументаЗаявкиНаРасходованиеДС = НайтиОбъектСинхронизации(КодВПрограмме,Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств"));
					
					Если ДокументаЗаявкиНаРасходованиеДС = Неопределено Тогда
						РезультатОбработки = "Не найден документ";
					Иначе
						ДокОбъект = ДокументаЗаявкиНаРасходованиеДС.ПолучитьОбъект();
					КонецЕсли;
				КонецЕсли;
			Исключение
				Ошибки.Добавить("ЗапросНаУдалениеЗаявкиНаРасходованиеДС. Получение объекта. " + ОписаниеОшибки() + " (" + УИДФайла + ")");
			КонецПопытки;
			...
			Если НЕ ЗначениеЗаполнено(РезультатОбработки) Тогда
				...
				Результат = ЗаполнитьДанныеОбОплатеЗаявки(ДокументаЗаявкиНаРасходованиеДС);
				
				Если Результат = "Не оплачена" Тогда
					Попытка
					...
					Исключение
						Ошибки.Добавить("ЗапросНаУдалениеЗаявкиНаРасходованиеДС. Удаление объекта. " + ОписаниеОшибки() + " (" + УИДФайла + ")");				
						РезультатОбработки = "Ошибка удаления." + ОписаниеОшибки();
					КонецПопытки;
				Иначе
			...
			Если ЗначениеЗаполнено(РезультатОбработки) Тогда
				...
				КорневойDOM.УстановитьАтрибут("ТипДокумента","ЗапросНаУдалениеЗаявкиНаРасходованиеДС");
				
				ТекстСообщения = "";
				
				Элемент = ДокументDOM.СоздатьЭлемент("КодВПрограмме");
				Элемент.ТекстовоеСодержимое = XMLСтрока(КодВПрограмме);
				КорневойDOM.ДобавитьДочерний(Элемент);
				
				Элемент = ДокументDOM.СоздатьЭлемент("СтатусУдаления");
				Элемент.ТекстовоеСодержимое = XMLСтрока(СтатусУдаления);
				КорневойDOM.ДобавитьДочерний(Элемент);

				Элемент = ДокументDOM.СоздатьЭлемент("Пользователь");
				Элемент.ТекстовоеСодержимое = XMLСтрока(Пользователь);
				КорневойDOM.ДобавитьДочерний(Элемент);

				Элемент = ДокументDOM.СоздатьЭлемент("РезультатОперации");
				Элемент.ТекстовоеСодержимое = XMLСтрока(РезультатОбработки);
				КорневойDOM.ДобавитьДочерний(Элемент);
...
Функция ЗаполнитьДанныеОбОплатеЗаявки(Заявка)
	...
	Если ФактическаяОплата Тогда
		Если Не ЗначениеЗаполнено(ТекстНеОплачено) Тогда
			Результат = "Оплачена полностью";
		Иначе
			Результат = "Оплачена частично";
		КонецЕсли;
	Иначе
		Результат = "Не оплачена";
	КонецЕсли;
...
Функция XML_Справочник_Контрагенты(Элементы, Ошибки, УИДФайла)	
	...
	Контрагент = НайтиОбъектСинхронизации(Результат.КодВПрограмме,Тип("СправочникСсылка.Контрагенты"));
	...
	Попытка
		...
		Контрагент.Записать();
		ЗаписатьУИД(Результат.КодВПрограмме, Контрагент.Ссылка);
...
Функция XML_Справочник_Договора(Элементы, Организация, Контрагент, Валюта, Ошибки, УИДФайла)
		...
		Если элемент.ИмяУзла = "КлючевыеСвойства" Тогда
			Для Каждого j Из элемент.ДочерниеУзлы Цикл
				...
				Попытка
					Результат.Вставить(j.ИмяУзла,XMLСтрока(j.ТекстовоеСодержимое));
				Исключение
					Ошибки.Добавить("Справочник.Договоры, ошибка разбора тегов файла. Имя узла " + j.ИмяУзла + ", ТекстовоеСодержимое = " + j.ТекстовоеСодержимое + ". (" + УИДФайла + "). " + ОписаниеОшибки());
				КонецПопытки;
			КонецЦикла;
			Продолжить;
		КонецЕсли;
	...
	ДоговорКонтрагента.ВалютаВзаиморасчетов = НайтиВалюту(Результат.Валюта);
	ДоговорКонтрагента.СпособДоставки = Перечисления.СпособыДоставки.ОтОтправителяОпределяетСлужбаДоставки;
	...
	ВалютаВзаиморасчетов = Рубли;
	Если Результат.Свойство("ВалютаВзаиморасчетов") И ЗначениеЗаполнено(Результат.ВалютаВзаиморасчетов) Тогда
		ВалютаВзаиморасчетов = НайтиВалюту(Результат.ВалютаВзаиморасчетов);
	КонецЕсли;
	
	Если ДоговорКонтрагента.ВалютаВзаиморасчетов = Рубли И ВалютаВзаиморасчетов = Рубли Тогда
		ДоговорКонтрагента.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВРублях;
	ИначеЕсли ДоговорКонтрагента.ВалютаВзаиморасчетов = Рубли И ВалютаВзаиморасчетов <> Рубли Тогда
	...
	ИначеЕсли ДоговорКонтрагента.ВалютаВзаиморасчетов <> Рубли И ВалютаВзаиморасчетов = Рубли Тогда
		ДоговорКонтрагента.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте;
	КонецЕсли;
...
Функция XML_Справочник_СоглашениеСКлиентом(Элементы, Организация, Контрагент, Валюта, Ошибки, УИДФайла)
	...
	ДоговорКонтрагентаСсылка = НайтиОбъектСинхронизации(Результат.КодВПрограмме,Тип("СправочникСсылка.СоглашенияСКлиентами"));
		
	Если ДоговорКонтрагентаСсылка <> Неопределено Тогда
		ДоговорКонтрагента = ДоговорКонтрагентаСсылка.ПолучитьОбъект();
	Иначе
		Соглашение = Справочники.СоглашенияСКлиентами.СоздатьЭлемент();
	КонецЕсли;	
	
	Соглашение.Валюта = НайтиВалюту(Результат.Валюта);
	
	Рубли = Справочники.Валюты.НайтиПоКоду("643");
	
	Если НЕ ЗначениеЗаполнено(Соглашение.Валюта) Тогда	
		Соглашение.Валюта = Рубли;
	КонецЕсли;
	
	Соглашение.ВалютаВзаиморасчетов = Рубли;
	Если Результат.Свойство("ВалютаВзаиморасчетов") И ЗначениеЗаполнено(Результат.ВалютаВзаиморасчетов) Тогда
		Соглашение.ВалютаВзаиморасчетов = НайтиВалюту(Результат.ВалютаВзаиморасчетов);
	КонецЕсли;
		
	Если Соглашение.Валюта = Рубли И Соглашение.ВалютаВзаиморасчетов = Рубли Тогда
			Соглашение.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВРублях;
	ИначеЕсли Соглашение.Валюта = Рубли И Соглашение.ВалютаВзаиморасчетов <> Рубли Тогда
		Соглашение.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВРублях;
	ИначеЕсли Соглашение.Валюта <> Рубли И Соглашение.ВалютаВзаиморасчетов = Рубли Тогда
		Соглашение.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте;			
	ИначеЕсли Соглашение.Валюта <> Рубли И Соглашение.ВалютаВзаиморасчетов <> Рубли Тогда
	...
	Соглашение.Партнер = Контрагент.Партнер;
	Соглашение.Контрагент = Контрагент;
...
Процедура XML_Справочник_Номенклатура(Элементы, ВидНоменклатуры, Ошибки, УИДФайла)
	...
	Попытка
		...
		Если НоменклатураСсылка.ПолучитьОбъект() <> Неопределено Тогда
			Возврат;
		КонецЕсли;
	Исключение
		УИД = Неопределено
...
Процедура XML_Документ_СчетОплатыПоставщику(Элементы, Организация, Контрагент, ДоговорСоглашения, Валюта, ВидНоменклатуры, Ошибки, УИДФайла)
	...
	КонецЕсли;
	Если РезультатШапка.Свойство("Приоритет") Тогда
		Док.Приоритет = Справочники.Приоритеты.НайтиПоНаименованию(РезультатШапка.Приоритет);
	КонецЕсли;
...
Процедура XML_Документ_ЗаявкаНаРасходованиеДС(Элементы, Организация, Контрагент, Договор, Валюта, Ошибки, УИДФайла)
	...
	ДокСсылка = НайтиОбъектСинхронизации(РезультатШапка.КодВПрограмме,Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств"));
	...
	КонецЕсли;
		...
		Если РезультатШапка.Свойство("ВалютаСчета") И ЗначениеЗаполнено(РезультатШапка.ВалютаСчета) Тогда
			ВалютаСчета = Справочники.Валюты.НайтиПоКоду(РезультатШапка.ВалютаСчета);
		КонецЕсли;
		
		Попытка
			Если ЗначениеЗаполнено(ВалютаСчета) Тогда
				Если ВалютаСчета <> Договор.ВалютаВзаиморасчетов Тогда
					ДоговорОбъект = Договор.ПолучитьОбъект();
					ДоговорОбъект.ВалютаВзаиморасчетов = ВалютаСчета;
					
					Рубли = Справочники.Валюты.НайтиПоКоду("643");
										
					Если ДоговорОбъект.ВалютаВзаиморасчетов = Рубли И ДоговорОбъект.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте Тогда
						ДоговорОбъект.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВРублях;
					ИначеЕсли ДоговорОбъект.ВалютаВзаиморасчетов = Рубли И ДоговорОбъект.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте Тогда
						ДоговорОбъект.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВРублях;
					ИначеЕсли ДоговорОбъект.ВалютаВзаиморасчетов <> Рубли И ДоговорОбъект.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВРублях Тогда
						ДоговорОбъект.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте;
					ИначеЕсли ДоговорОбъект.ВалютаВзаиморасчетов <> Рубли И ДоговорОбъект.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВРублях Тогда	
						ДоговорОбъект.ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте;
					КонецЕсли;
					
					ДоговорОбъект.Записать();
				КонецЕсли;
			Иначе
				ВалютаСчета = Договор.ВалютаВзаиморасчетов;
			КонецЕсли;
			
			Док.Валюта = ВалютаСчета;
		...
		Если РезультатШапка.Свойство("Приоритет") Тогда
			Док.ПриоритетОплаты = Справочники.ПриоритетыОплаты.НайтиПоНаименованию(РезультатШапка.Приоритет);
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Док.ПриоритетОплаты) Тогда
		...
		Если ЗначениеЗаполнено(Договор) Тогда
			НоваяСтрока.ВалютаВзаиморасчетов = Договор.ВалютаВзаиморасчетов;	
		КонецЕсли;
		НоваяСтрока.Организация = Организация;
		НоваяСтрока.Партнер = Контрагент.Партнер;
		НоваяСтрока.СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.ОплатаПоставщику;
		
		Если РезультатШапка.Свойство("Комментарий") Тогда
			НоваяСтрока.Комментарий = РезультатШапка.Комментарий;
		КонецЕсли;
...
Процедура XML_Документ_ПриходныйОрдерНаТовары(Элементы, Организация, Контрагент, Склад, Валюта, Ошибки, УИДФайла)
	...
	Если Док = Неопределено Тогда
		...
		Док.Дата = РезультатШапка.Дата;
		Док.Номер = РезультатШапка.Номер;
		Док.Отправитель = Контрагент.Партнер;
		Попытка
			Док.Статус = Перечисления.СтатусыПриходныхОрдеров[РезультатШапка.Статус];
		Исключение
			Ошибки.Добавить("Статус ПриходныйОрдерНаТовары - " + РезультатШапка.Статус + " " + ОписаниеОшибки() + " (" + УИДФайла + ")");
...
Процедура XML_Документ_ТаможеннаяДекларация(Элементы, Организация, Контрагент, ДоговорСоглашения, Валюта, Ошибки, УИДФайла)
	...
	ДокОбъект.Поставщик = Контрагент.Партнер;
	ДокОбъект.КонтрагентПоставщика = Контрагент;
...
Процедура XML_Документ_СчетПокупателю(Элементы, Организация, Контрагент, Договор, Валюта, Ошибки, УИДФайла) 
	...
	Иначе
		...
		Иначе
			Док = Документы.ЗаказКлиента.СоздатьДокумент();
			Запись = РегистрыСведений.Расш1_СоответствияКлик1С.СоздатьМенеджерЗаписи();
			Запись.УИД = РезультатШапка.КодВПрограмме;
		 	Запись.Ресурс = Тип("ДокументСсылка.ЗаказКлиента");
			Запись.Удалить();
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		...
		ПроверкаЗаписи = Истина;		
	КонецПопытки;
	
	Если ПроверкаЗаписи Тогда
		Попытка
			Док.Записать(РежимЗаписиДокумента.Запись);
		Исключение
			Ошибки.Добавить("Запись документа ЗаказКлиента " + ОписаниеОшибки() + " (" + УИДФайла + ")");
...
Функция НайтиОбъектСинхронизации(УИДТекст, ТипДокумента = Неопределено)
	...
	Запрос.УстановитьПараметр("УИД", УИДТекст);
...
//		НовыйКонтрагент.КПП = Данные.КПП;
		...
		Набор.Ресурс = НовыйКонтрагент.Ссылка;
		Набор.Записать();
		
		Запрос.УстановитьПараметр("КПП", НовыйКонтрагент.КПП);
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Пока ВыборкаДетальныеЗаписиКБанкам.Следующий() Цикл
			 ...
			 ЭлементД.ТекстовоеСодержимое = XMLСтрока(Данные.КодВПрограмме);
 			 ЭлементБанка.ДобавитьДочерний(ЭлементД);
		...
		ТипКонтрагента = Новый ТаблицаЗначений;
		ТипКонтрагента.Колонки.Добавить("Тип");		
		...
		Пока ВыборкаДетальныеЗаписиДоговора.Следующий() Цикл
			 ...
			 НоваяСтрока = ТипКонтрагента.Добавить();
			 НоваяСтрока.Тип = Строка(ВыборкаДетальныеЗаписиДоговора.ТипДоговора);
		...
		ТипКонтрагента.Свернуть("Тип");
		ТипКонтрагента.Сортировать("Тип");
		ТипКонрагентаСтрока = "";
		
		Для Каждого Э Из ТипКонтрагента Цикл
			Если ЗначениеЗаполнено(ТипКонрагентаСтрока) Тогда
				ТипКонрагентаСтрока = ТипКонрагентаСтрока + ", ";
			КонецЕсли;
			ТипКонрагентаСтрока = ТипКонрагентаСтрока + Э.Тип;
		КонецЦикла;
		
		ЭлементД = ДокументDOM.СоздатьЭлемент("ТипКонтрагента");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ТипКонрагентаСтрока);
		КорневойDOM.ДобавитьДочерний(ЭлементД);
		...
		ЭлементД = ДокументDOM.СоздатьЭлемент("GUID1C");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(Строка(ВыборкаДетальныеЗаписи.Ссылка.УникальныйИдентификатор()));
		КорневойDOM.ДобавитьДочерний(ЭлементД);
		
		ЭлементД = ДокументDOM.СоздатьЭлемент("ver.");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока("1.0.0.9");
		КорневойDOM.ДобавитьДочерний(ЭлементД);
		
		//ЭлементКонтрагент.УстановитьАтрибут("КодВПрограмме",Строка(ВыборкаДетальныеЗаписи.Ссылка.УникальныйИдентификатор()));
		
		Если Данные.Свойство("Пользователь") Тогда
			ЭлементД = ДокументDOM.СоздатьЭлемент("Пользователь");
			ЭлементД.ТекстовоеСодержимое = XMLСтрока(Данные.Пользователь);
			КорневойDOM.ДобавитьДочерний(ЭлементД);
		КонецЕсли;
...
Процедура ОтправитьБанковскиеРеквизитыКонтрагентов() Экспорт
	Адрес = "125.62.28.226";
	Порт = 5672;
	Логин = "admin";
	Пароль = "1qaz2wsX";
	...
	ДокументDOM = Новый ДокументDOM(ИмяПространства, "Message");
	КорневойDOM = ДокументDOM.ЭлементДокумента;
	КорневойDOM.УстановитьАтрибут("ТипДокумента","Справочник.БанковскиеСчетаКонтрагентов");
	...
	Запрос.Текст = 
		...
		|		ПО Контрагенты.Партнер = ПартнерыСегмента.Партнер
		|ГДЕ
		...
		|	БанковскиеСчетаКонтрагентов.Наименование КАК Наименование,
		|	БанковскиеСчетаКонтрагентов.НомерСчета КАК НомерСчета,
		|	БанковскиеСчетаКонтрагентов.Банк.Ссылка КАК БанкСсылка,
		|	БанковскиеСчетаКонтрагентов.БанкДляРасчетов.Ссылка КАК БанкДляРасчетовСсылка,
		...
		|	Справочник.КлассификаторБанков КАК КлассификаторБанков
		|ГДЕ
		|	КлассификаторБанков.Ссылка В
		|			(ВЫБРАТЬ
		|				ВТ_РС.БанкСсылка КАК БанкСсылка
		|			ИЗ
		|				ВТ_РС КАК ВТ_РС)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		...
		|	КлассификаторБанков.Страна.Код
		|ИЗ
		|	Справочник.КлассификаторБанков КАК КлассификаторБанков
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_РС.Ссылка КАК Ссылка,
		|	ВТ_РС.Наименование КАК Наименование,
		|	ВТ_РС.НомерСчета КАК НомерСчета,
		|	ВТ_РС.БанкСсылка КАК БанкСсылка,
		|	ВТ_РС.БанкДляРасчетовСсылка КАК БанкДляРасчетовСсылка,
		|	ВТ_РС.ТекстКорреспондента КАК ТекстКорреспондента,
		|	ВТ_РС.ТекстНазначения КАК ТекстНазначения,
		|	ВТ_РС.ВалютаДенежныхСредствКод КАК ВалютаДенежныхСредствКод,
		|	ВТ_РС.Владелец КАК Владелец
		|ИЗ
		|	ВТ_РС КАК ВТ_РС";
	
	Сегмент = Справочники.СегментыПартнеров.НайтиПоНаименованию("Бизнес");
	...
	БанкиОрг  = ДокументDOM.СоздатьЭлемент("Справочник.Банки");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЭлементД = ДокументDOM.СоздатьЭлемент("Наименование");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.Наименование);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);

		ЭлементД = ДокументDOM.СоздатьЭлемент("БИК");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.Код);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
			 
		ЭлементД = ДокументDOM.СоздатьЭлемент("КоррСчет");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.КоррСчет);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);

		ЭлементД = ДокументDOM.СоздатьЭлемент("SWIFT");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.СВИФТБИК);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
		...
		ЭлементД = ДокументDOM.СоздатьЭлемент("АдресБанка");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.Адрес);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);

		ЭлементД = ДокументDOM.СоздатьЭлемент("КодСтраны");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.СтранаКод);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
	...
	БанкиОрг  = ДокументDOM.СоздатьЭлемент("Справочник.БанковскиеСчета");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЭлементД = ДокументDOM.СоздатьЭлемент("Банк");
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.БанкСсылка) Тогда
			ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.БанкСсылка.УникальныйИдентификатор());
		КонецЕсли;
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
		...
		ЭлементД = ДокументDOM.СоздатьЭлемент("РасчетныйСчет");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.НомерСчета);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
		
		ЭлементД = ДокументDOM.СоздатьЭлемент("БанкДляРасчетов");
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.БанкДляРасчетовСсылка) Тогда
			ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.БанкДляРасчетовСсылка.УникальныйИдентификатор());
		КонецЕсли;
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
		...
		ЭлементД = ДокументDOM.СоздатьЭлемент("ВалютаСчета");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.ВалютаДенежныхСредствКод);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
			 
		ЭлементД = ДокументDOM.СоздатьЭлемент("ТекстКорреспондента");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.ТекстКорреспондента);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);

		ЭлементД = ДокументDOM.СоздатьЭлемент("ТекстНазначения");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.ТекстНазначения);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
		
		ЭлементД = ДокументDOM.СоздатьЭлемент("Контрагент");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.Владелец.УникальныйИдентификатор());
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
