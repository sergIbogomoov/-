...
Функция XML_Справочник_Договора(Элементы, Организация, Контрагент, Валюта, Ошибки, УИДФайла)
	...
	Если ДоговорКонтрагента.ВалютаВзаиморасчетов.Код = "643" Тогда
...
Функция XML_Справочник_СоглашениеСКлиентом(Элементы, Организация, Контрагент, Валюта, Ошибки, УИДФайла)
	...
	Если Соглашение.Валюта.Код = "643" Тогда
		...
		КонецЕсли;
	Иначе
		Если Соглашение.ВалютаВзаиморасчетов.Код = "643" Тогда
...
Процедура XML_Документ_ЗаявкаНаРасходованиеДС(Элементы, Организация, Контрагент, Договор, Валюта, Ошибки, УИДФайла)
	...
	Если Док = Неопределено Тогда
		...
		Док.Организация = Организация;
		Попытка
			Док.Валюта = Договор.Валюта;
		Исключение
			Ошибки.Добавить("Ошибка Договор.Валюта ЗаявкаНаРасходованиеДенежныхСредств. " + Договор + ". " + ОписаниеОшибки() + " (" + УИДФайла + ")");
		КонецПопытки;
		...
		Док.БанковскийСчетКонтрагента = НайтиСчетОрганизации(Док.Организация,Валюта);
		Док.ФормаОплатыБезналичная = Истина;
...
Функция НайтиСчетОрганизации(Организация,Валюта)
...
Процедура ОтправитьБанковскиеРеквизитыКонтрагентов() Экспорт
	...
	Запрос.Текст = 
		...
		|	БанковскиеСчетаКонтрагентов.СтранаБанка.Код КАК СтранаБанкаКод
		|ПОМЕСТИТЬ ВТ_РС
		|ИЗ
		|	Справочник.БанковскиеСчетаКонтрагентов КАК БанковскиеСчетаКонтрагентов
		|ГДЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_РС.БИКБанка КАК БИКБанка,
		|	ВТ_РС.НаименованиеБанка КАК НаименованиеБанка,
		|	ВТ_РС.КоррСчетБанка КАК КоррСчетБанка,
		|	ВТ_РС.ГородБанка КАК ГородБанка,
		|	ВТ_РС.АдресБанка КАК АдресБанка,
		|	ВТ_РС.СВИФТБанка КАК СВИФТБанка,
		|	ВТ_РС.СтранаБанкаКод КАК СтранаБанкаКод,
		|	ВТ_РС.Ссылка КАК Ссылка
		|ИЗ
		|	ВТ_РС КАК ВТ_РС
		|ГДЕ
		|	ЕСТЬNULL(ВТ_РС.БанкСсылка, 0) = 0";
	...
	СписокУИДБанков = Новый ТаблицаЗначений;
	СписокУИДБанков.Колонки.Добавить("Ссылка");
	СписокУИДБанков.Колонки.Добавить("УИД");
	
	ВыборкаДетальныеЗаписи = РезультатЗапроса[4].Выбрать();	
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НоваяСтрока = СписокУИДБанков.Добавить();
		...
		ЭлементД = ДокументDOM.СоздатьЭлемент("КодВПрограмме");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(НоваяСтрока.УИД);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
			 
		ЭлементД = ДокументDOM.СоздатьЭлемент("Наименование");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.НаименованиеБанка);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);

		ЭлементД = ДокументDOM.СоздатьЭлемент("БИК");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.БИКБанка);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
			 
		ЭлементД = ДокументDOM.СоздатьЭлемент("КоррСчет");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.КоррСчетБанка);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);

		ЭлементД = ДокументDOM.СоздатьЭлемент("SWIFT");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.СВИФТБанка);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
		...
		ЭлементД = ДокументDOM.СоздатьЭлемент("АдресБанка");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.АдресБанка);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);

		ЭлементД = ДокументDOM.СоздатьЭлемент("КодСтраны");
		ЭлементД.ТекстовоеСодержимое = XMLСтрока(ВыборкаДетальныеЗаписи.СтранаБанкаКод);
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			НайденнаяСтрока = СписокУИДБанков.НайтиСтроки(Отбор);
			Если НайденнаяСтрока.Количество() > 0 Тогда
				ЭлементД.ТекстовоеСодержимое = XMLСтрока(НайденнаяСтрока[0].УИД);
			КонецЕсли;
		КонецЕсли;
 		ЭлементБанка.ДобавитьДочерний(ЭлементД);
